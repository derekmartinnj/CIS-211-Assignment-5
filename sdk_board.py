...
class Tile(Listenable):
    ...
    def __repr__(self):
        return f"Tile({self.row}, {self.col}, '{self}')"
...
class Board(object):
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            empty = set()
            for tile in group:
                if tile.value != UNKNOWN and tile.value in CHOICES:
                    empty.add(tile.value)
            for tile in group:
                if tile.value == UNKNOWN:
                    if tile.remove_candidates(empty):
                        var = True
        return var
    ...
    def solve(self) -> bool:
        ...
        tile_choice = self.min_choice_tile()
        for candidate in tile_choice.candidates:
            tile_choice.set_value(candidate)
            if self.solve():
                return True
            else:
